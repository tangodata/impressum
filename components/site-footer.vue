...
const { footmenu: menu } = useAppConfig()
</script>

<template lang="pug">
//pre {{ menu  }}
// =========={ FOOTER }==========
footer.bg-black.text-gray-400.mt-4
  // Footer content
  #footer-content.relative.pt-8.pb-6(class="xl:pt-16 xl:pb-12")
    .mx-auto.px-3.overflow-hidden(class="xl:container sm:px-4 xl:px-2")
      .flex.flex-wrap.flex-row.-mx-3(class="lg:justify-between")
        .flex-shrink.max-w-full.w-full.px-3(class="lg:w-2/5 lg:pr-16")
          .flex.items-center.mb-2
            span.text-3xl.leading-normal.mb-2.font-bold.text-gray-100.mt-2 Tangueros
            // <img src="src/img-min/logo.png" alt="LOGO">
          p: i We Love Argentine Tango.


          ul.space-x-3.mt-6.mb-6(class="Lg:mb-0")
            // facebook
            li.inline-block
        ...
        .flex-shrink.max-w-full.w-full.px-3(class="lg:w-3/5")
          .flex.flex-wrap.flex-row
            template(v-for="bunch in menu")
              .flex-shrink.max-w-full.mb-6(class="w-1/2 md:w-1/4 lg:mb-0")
                h4.text-sm.leading-normal.mb-0.uppercase.text-gray-100
                  | {{ bunch.groupname }}
                ul.text-xs
                  template(v-for="item in bunch.items")
                    li.py-1(class="hover:text-white")
                      a(:href="item.link") {{ item.name }}

  // Start footer copyright
  .footer-dark
    .container.py-4.border-t.border-gray-200.border-opacity-10
      .row
        .col-12.col-md.text-center
          p.d-block.my-3 Copyright © Tango East | All rights reserved.
  // End footer copyright
// end footer


// =========={ SCROLL TO TOP }==========
a.back-top.fixed.p-4.rounded.bg-gray-100.border.border-gray-100.text-gray-500.right-4.bottom-4.hidden(
  href="#" class="dark:bg-gray-900 dark:border-gray-800"
